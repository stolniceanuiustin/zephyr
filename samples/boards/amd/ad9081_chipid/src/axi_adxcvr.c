...
int axi_adxcvr_fi_break_refclk(const struct device *dev)
{
	...
	/*
	 * Point the GT at QPLL1, which this bitstream does not drive. The GT
	 ...
	 */
	if (cfg->sys_clk_sel == ADXCVR_SYS_CLK_QPLL1) {
		LOG_ERR("FI: %s already selects QPLL1 -- nothing to break",
			dev->name);
		return -EINVAL;
	}
	...
	control = (cfg->lpm_enable ? ADXCVR_LPM_DFE_N : 0) |
		  ADXCVR_SYSCLK_SEL(ADXCVR_SYS_CLK_QPLL1) |
		  ADXCVR_OUTCLK_SEL(cfg->out_clk_sel);
	adxcvr_write(dev, ADXCVR_REG_CONTROL, control);

	LOG_WRN("FI: %s sysclk forced to QPLL1 (undriven), CONTROL=0x%04x",
		dev->name, control);
	return 0;
}

/*
 * Which PLL a sys_clk_sel value names. Only for the log line below, which the
 * golden FI log was captured with -- when the transceiver was one hardcoded RX
 * singleton the name was a literal, and it has to stay printed as a name.
 */
static const char *adxcvr_sysclk_name(uint8_t sys_clk_sel)
{
	switch (sys_clk_sel) {
	case XCVR_CPLL:
		return "CPLL";
...
int axi_adxcvr_fi_restore_refclk(const struct device *dev)
{
	const struct adxcvr_config *cfg = dev->config;
	struct adxcvr *x = dev->data;
	uint32_t control = adxcvr_control_word(dev);

	adxcvr_write(dev, ADXCVR_REG_RESETN, 0);
	adxcvr_write(dev, ADXCVR_REG_CONTROL, control);

	/*
	 * The DRP dividers were programmed against the configured PLL at
	 * configure time and the break above did not touch them, so restoring
	 * the mux is enough -- but re-solve them anyway rather than assume,
	 * since a failed lock attempt is not a state this code has any right to
	 * make assumptions about.
	 */
	LOG_INF("FI: %s sysclk restored to %s, CONTROL=0x%04x", dev->name,
		adxcvr_sysclk_name(cfg->sys_clk_sel), control);
	return adxcvr_clk_set_rate(dev, cfg->lane_rate_khz, x->ref_rate_khz);
}

uint32_t axi_adxcvr_fi_status(const struct device *dev)
{
	return adxcvr_read(dev, ADXCVR_REG_STATUS);
}
