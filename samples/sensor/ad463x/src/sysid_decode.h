 ...
 *
 * The AXI System ID core is exposed by Zephyr as a read-only EEPROM (the
 * transport). The ROM *format* — header, build-info records and checksums —
 * is decoded here, on top of the standard eeprom_read() API, so the ROM-format
 ...
 */
static inline int sysid_read_str(const struct device *eeprom, off_t offset,
				 char *dst, size_t dst_size)
{
	size_t rom_size = eeprom_get_size(eeprom);

	dst[0] = '\0';

	if (offset == 0 || (size_t)offset >= rom_size) {
		return 0;
	}

	for (size_t i = 0; i < dst_size - 1 && (size_t)offset + i < rom_size; i++) {
		char c;
		int ret = eeprom_read(eeprom, offset + i, &c, 1);
 ...
 * read-only EEPROM device. Returns 0 on success, negative errno on failure.
 */
static inline int sysid_get_board_info(const struct device *eeprom,
				       struct sysid_board_info *info)
{
	...
	ret = eeprom_read(eeprom, 0, &header, sizeof(header));
	if (ret < 0) {
		return ret;
	}
	...
	if (header.version != SYSID_HEADER_V1 &&
	    header.version != SYSID_HEADER_V2) {
		return -ENOTSUP;
	}
	info->header_version = header.version;

	ret = sysid_read_str(eeprom, (off_t)header.board_info_offs * sizeof(uint32_t),
			     info->board, sizeof(info->board));
	if (ret < 0) {
		return ret;
	}

	ret = sysid_read_str(eeprom, (off_t)header.product_info_offs * sizeof(uint32_t),
	...
	if (header.version == SYSID_HEADER_V2) {
		...
		if ((size_t)build_off + sizeof(build) > rom_size) {
			return -EFAULT;
		}
		ret = eeprom_read(eeprom, build_off, &build, sizeof(build));
		if (ret < 0) {
			return ret;
		}
