...
/* Send exactly len bytes, looping over partial writes. Returns 0 on success,
...
static int send_all(const uint8_t *buf, int len)
{
    ...
    while (sent < len) {
        ...
        if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            /* TX path momentarily full — let lower-priority net threads run. */
            k_msleep(1);
            continue;
        }
...
void fb_udp_send_frame(const uint32_t *pixels, int width, int height)
{
    ...
    uint32_t total_bytes = (uint32_t)width * height * 4;
    ...
    if (send_all(hdr, LEN_HEADER_SIZE) < 0 ||
        send_all((const uint8_t *)pixels, total_bytes) < 0) {
