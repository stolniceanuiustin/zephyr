...
/**
 * @brief Callback success return: this device's work for the phase is done.
 *
 * A state-op callback returns this (1) or 0 on success; only a negative errno is
 * a failure. The FSM is best-effort and counts failures rather than aborting, so
 * a phase with nothing to do -- an enable phase on UNINIT, say -- also returns
 * this rather than reporting an error.
 */
 ...
 * the board's bring-up file for what the current tables really unwind.
 */
