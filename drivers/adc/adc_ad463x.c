...
#include <zephyr/drivers/misc/adi/spi_engine_offload.h>
...
static const struct ad463x_part_info ad463x_parts[] = {
	{ "ad4630-24", AD463X_ID_AD4630_24, 24 },
	{ "ad4630-20", AD463X_ID_AD4630_20, 20 },
	{ "ad4630-16", AD463X_ID_AD4630_16, 16 },
	{ "ad4631-24", AD463X_ID_AD4631_24, 24 },
	{ "ad4631-20", AD463X_ID_AD4631_20, 20 },
	{ "ad4631-16", AD463X_ID_AD4631_16, 16 },
	{ "ad4632-24", AD463X_ID_AD4632_24, 24 },
	{ "ad4632-20", AD463X_ID_AD4632_20, 20 },
 ...
 */
static void ad463x_dma_cb(const struct device *dma_dev, void *user_data,
 ...
 */
static int ad463x_dma_arm(const struct device *dev, void *buf, uint32_t bytes,
			  bool cyclic)
{
...
static int ad463x_spi_reg_write_masked(const struct device *dev,
...
static int ad463x_set_avg_frame_len(const struct device *dev, uint8_t mode)
{
	...
	if (mode == 0) {
		...
		return ad463x_spi_reg_write(dev, AD463X_REG_AVG,
 ...
 * configured lane / output / data-rate. Same logic as no-OS ad463x_init().
 *
...
static int ad463x_apply_capture_config(const struct device *dev)
{
	...
	ret = ad463x_spi_reg_write_masked(dev, AD463X_REG_MODES,
					   AD463X_LANE_MODE_MSK, cfg->lane_mode);
	if (ret) {
		LOG_ERR("MODES.lane_mode write failed (%d)", ret);
		return ret;
	}
	ret = ad463x_spi_reg_write_masked(dev, AD463X_REG_MODES,
					   AD463X_CLK_MODE_MSK, cfg->clock_mode);
	if (ret) {
		LOG_ERR("MODES.clock_mode write failed (%d)", ret);
		return ret;
	}
	ret = ad463x_spi_reg_write_masked(dev, AD463X_REG_MODES,
					   AD463X_DDR_MODE_MSK, cfg->data_rate);
	if (ret) {
	...
	if (cfg->avg_frame_len > 0) {
		...
		if (ret) {
			LOG_ERR("avg_frame_len=%u write failed (%d)",
				cfg->avg_frame_len, ret);
			return ret;
		}
		LOG_INF("averaging enabled: %u conversions → 30-bit output",
			cfg->avg_frame_len);
	}
...
{
	return ad463x_spi_reg_write(dev, AD463X_REG_EXIT_CFG_MODE,
...
int ad463x_init_chip(const struct device *dev)
{
	...
	ret = clock_control_set_rate(cfg->clkgen, CLOCK_CONTROL_SUBSYS_ALL,
				     ...
				     cfg->clkgen_rate_hz);
	if (ret) {
		LOG_ERR("clkgen_set_rate(%u) failed (%d)",
			cfg->clkgen_rate_hz, ret);
		return ret;
	}
	...
	ret = ad463x_spi_reg_write(dev, AD463X_REG_SCRATCH_PAD,
				    AD463X_SCRATCH_TEST);
	if (ret) {
	...
	if (scratch != AD463X_SCRATCH_TEST) {
		LOG_ERR("scratchpad read 0x%02x, expected 0x%02x — chip not responding",
			scratch, AD463X_SCRATCH_TEST);
		return -ENODEV;
	}
...
static int ad463x_read_data(const struct device *dev, uint32_t *buf, uint16_t samples)
{
	 ...
	 */
	commands[cmd_idx++] =
		SPI_ENGINE_CMD_BUILD_CONFIG_CLK_DIV(0);
	commands[cmd_idx++] =
	...
	(void)pwm_set(cfg->pwmgen, cfg->pwmgen_channel, 0, 0,
		      PWM_POLARITY_NORMAL);
	(void)pwm_set(cfg->pwmgen, cfg->pwmgen_channel + 1, 0, 0,
		      PWM_POLARITY_NORMAL);
out_offload:
...
ssize_t ad463x_read_buffer(const struct device *dev, void *buf, size_t len)
{
	...
	if (max_samples == 0) {
		LOG_ERR("buffer too small: %zu B < one sample (%u B)",
			len, data->bytes_per_sample);
		k_mutex_unlock(&data->lock);
		return -ENOMEM;
	}
...
static int ad463x_continuous_start(const struct device *dev)
{
	...
	k_mutex_unlock(&data->lock);
	LOG_INF("AD463x continuous capture armed at %u Hz",
		cfg->sample_rate_hz);
	return 0;
}
...
/*
 * Snapshot the most recent sample(s) out of the cyclic ring.
 */
static int ad463x_adc_read(const struct device *dev,
...
static int ad463x_init(const struct device *dev)
{
	...
	if (cfg->clock_mode == AD463X_SPI_COMPATIBLE_MODE &&
	    cfg->data_rate == AD463X_DDR_MODE) {
...
#define AD463X_DATA_RATE(node_id)						\
	(DT_ENUM_IDX(node_id, adi_data_rate) == 0 ? AD463X_SDR_MODE :		\
						     AD463X_DDR_MODE)
...
#define AD463X_INIT(n)								\
	...
	DEVICE_DT_INST_DEFINE(n, ad463x_init, NULL,				\
			      &ad463x_data_##n, &ad463x_config_##n,		\
			      POST_KERNEL,					\
