...
static inline void dmac_write(const struct device *dev, uint32_t reg,
			      uint32_t val)
{
	sys_write32(val, DEVICE_MMIO_GET(dev) + reg);
}

static int dmac_submit_burst(const struct device *dev,
			     struct axi_dmac_data *data)
{
	uint32_t burst_size;
	...
	if (data->direction == AXI_DMAC_DEV_TO_MEM ||
	...
	if (data->direction == AXI_DMAC_MEM_TO_DEV ||
	    data->direction == AXI_DMAC_MEM_TO_MEM) {
...
static int dmac_service(const struct device *dev, struct axi_dmac_data *data)
{
	...
	if (pending & AXI_DMAC_IRQ_EOT) {
		...
		if (data->saw_sot && data->remaining_size == 0 &&
...
static int axi_dmac_configure(const struct device *dev, uint32_t channel,
			      struct dma_config *cfg)
{
	...
	if (to_hw_dir(cfg->channel_direction) != data->direction) {
		LOG_ERR("requested direction %u != hardware direction %u",
			cfg->channel_direction, data->direction);
		return -EINVAL;
	}
	...
	if (data->width_src && (src_addr % data->width_src)) {
		LOG_ERR("src addr 0x%08x not aligned to %u bytes",
			src_addr, data->width_src);
		return -EINVAL;
	}

	if (data->width_dest && (dest_addr % data->width_dest)) {
		LOG_ERR("dest addr 0x%08x not aligned to %u bytes",
			dest_addr, data->width_dest);
		return -EINVAL;
	}
...
static int axi_dmac_start(const struct device *dev, uint32_t channel)
{
	...
	dmac_write(dev, AXI_DMAC_REG_IRQ_PENDING,
			       ...
			       struct dma_status *status)
{
	...
	if (!data->has_irq) {
		...
		if (event >= 0 && data->callback != NULL) {
			data->callback(dev, data->user_data, AXI_DMAC_CHANNEL,
				       event);
		}
	}
