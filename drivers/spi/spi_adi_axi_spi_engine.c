 ...
 * conversion streaming is exposed as a thin vendor extension in
 * <zephyr/drivers/misc/adi/spi_engine_offload.h> — Zephyr has no generic
 * SPI-offload framework, so that surface stays ADI-specific.
 *
...
/* Register and command encoding mirrors the no-OS reference driver. */
...
struct axi_spi_engine_data {
	...
	uint32_t sync_id;	/* matches no-OS _sync_id, increments per xfer */
};
 ...
 *
 * The SPI Engine is a "load program, then execute" core: push the full
 * command sequence (CLK_DIV, XFER_BITS, lane masks, CS assert, TRANSFER,
 * CS deassert, SYNC), stream all SDO words MSB-first, poll SYNC_ID, then
 * drain the SDI FIFO back into the rx buffer. tx and rx are tracked
 * independently by spi_context, so the SDO-push and SDI-pop passes can
 * each walk their own side of the (full-duplex) transfer.
 */
			       ...
			       const struct spi_config *config)
{
	...
	if (config->frequency == 0 ||
		...
		clk_div = cfg->ref_clock_hz / (2 * config->frequency) - 1;
		if (clk_div > 255) {
			clk_div = 255;
		}
	}
	...
	ret = spi_eng_write_cmd(dev,
		SPI_ENGINE_CMD_CONFIG(SPI_ENGINE_CMD_REG_CONFIG, 0));
	if (ret) {
		return ret;
	}
			     ...
			     const struct spi_config *config)
{
	...
	if (config->operation & (SPI_MODE_LOOP | SPI_TRANSFER_LSB)) {
		LOG_ERR("unsupported operation flags");
		return -ENOTSUP;
	}
...
/*
 * Offload extension (see <zephyr/drivers/misc/adi/spi_engine_offload.h>).
 * These drive the SPI Engine's autonomous offload module, which replays a
 * fixed command program on every external CNV trigger and streams the SDI
 * data to an AXI DMAC. This has no equivalent in the generic SPI API.
 */
...
static int axi_spi_engine_init(const struct device *dev)
{
	...
	/* Read hardware data width and number of SDI lanes (per Linux:
