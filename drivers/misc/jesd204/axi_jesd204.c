...
/* DATA must hold this long: a marginal TX eye flicks CGS->DATA->CGS, so one DATA read is not a lock. */
#define JESD204_LINK_HOLD_MS           12
...
int axi_jesd204_status_read(const struct device *dev)
{
	 ...
	 * up. Sampling once races that transition, and a marginal TX eye can flick
	 * into DATA then fall back to CGS -- accepting that first DATA read declares
	 * a lock that isn't there (link status_print then shows CGS, loopback dead).
	 * So poll until DATA has held for HOLD_MS consecutively, or timeout.
	 */
	int32_t waited_ms = 0;
	/* Consecutive DATA reads spanning HOLD_MS: N reads confirm (N-1)*STEP ms held. */
	const int data_reads_needed = JESD204_LINK_HOLD_MS / JESD204_LINK_SETTLE_STEP_MS + 1;
	int consecutive_data = 0;
	...
	for (;;) {
		...
		if ((status & 0x3) == JESD204_LINK_STATUS_DATA) {
			if (++consecutive_data >= data_reads_needed) {
				break;
			}
		} else {
			consecutive_data = 0; /* dropped out of DATA; restart the hold */
		}

		if (waited_ms >= JESD204_LINK_SETTLE_TIMEOUT_MS) {
			break;
		}
	...
	/* Success only when DATA (0x3) was held for the full hold window. */
	if (consecutive_data >= data_reads_needed) {
		LOG_INF("%s: DATA (settled in %d ms, held %d ms)", dev->name,
			waited_ms, JESD204_LINK_HOLD_MS);
		return 0;
	}
...
const struct jesd204_dev_data axi_jesd204_jesd204_data = {
	.max_num_links = 1,
	.state_ops = {
