 ...
 * comes from the AD9523-1 clock generator, which initialises earlier at
 * POST_KERNEL. This driver's scope is the standalone SPI bring-up only: prove
 * the bus by reading the product ID, soft-reset the chip, apply the
 * ad9144_setup() register sequence, and report the SERDES PLL lock status. It
 * does NOT wire the AXI JESD204 link cores, the transceivers, or the jesd204
 * FSM -- those are a separate later block, and until they land the actual DAC
 * output / lane sync is UNVERIFIED.
 *
 * Scoped to the DAQ2-exercised path only: pll_enable=false (external DAC clock,
 * on-chip PLL off), interpolation=1. The other variant branches, the DAC-PLL
 * setup, and the sample-rate machinery are dropped.
 *
 * The JESD204B ILAS configuration bytes are DERIVED from the devicetree geometry
 * rather than hardcoded, so a geometry that disagrees with the synthesised
 * bitstream cannot be set; each derived byte is BUILD_ASSERT-ed at build time so
 * a wrong encoding fails the build instead of reaching hardware.
 *
...
#define AD9144_REG_CDR_RESET		0x206
...
#define AD9144_REG_PCLK_CTRL		0x314
 ...
 * DAC while TXEN is low; DAQ2 leaves the TXEN EMIO GPIOs undriven (float low),
 * so override the pin -- take TXEN from SPI (ENA_SPI_TXEN) and drive it high
 * (SPI_TXEN).
 */
...
/*
 * SERDES CDR and receive-PLL dividers, selected by lane rate. DAQ2 (10 Gbps)
 * takes the >= 5.75 Gbps entry; lower-rate entries are carried for completeness.
 */
...
/* Fixed control words (datasheet-derived, single-use). */
...
/*
 * Lane crossbar: DAQ2/ZCU102 uses the identity map (PHY lane i = logical lane i).
 * It is a hardcoded board fact, not a DT knob.
 */
...
/*
 * ILAS config registers derived from the DT geometry (not hardcoded), each
 * BUILD_ASSERT-ed in AD9144_DEFINE so a wrong encoding fails the build. Byte
 * layouts (values shown for the DAQ2 profile):
 *
 *   SCR_L  (0x453): bit7 = scrambling, bits[4:0] = L-1        -> 0x83
 *   ILS_F  (0x454): F-1                                       -> 0x00
 *   ILS_K  (0x455): K-1                                       -> 0x1f
 *   ILS_M  (0x456): M-1                                       -> 0x01
 *   ILS_CS_N (0x457): bits[7:6]=CS, bits[4:0]=N-1            -> 0x0f
 *   ILS_NP (0x458): bits[7:5]=subclass, bits[4:0]=N'-1       -> 0x2f
 *   ILS_S  (0x459): bit5=JESDVER, bits[4:0]=S-1              -> 0x20
 *   ILS_HD_CF (0x45A): bit7 = high-density                   -> 0x80
 *
 * S = F*8*L/(M*N') and HD = (F*8 < N') are derived, not free parameters.
 */
...
/*
 * Required device configuration (datasheet Rev. B table 16). Opaque factory
 * settings with no per-register public semantics.
 */
...
/*
 * SERDES optimization (datasheet Rev. B table 39, 9144-specific pair).
 */
...
/*
 * Optimal SERDES PLL settings (datasheet Rev. B table 39). Opaque PLL tuning.
 */
...
/*
 * Block 1: soft-reset then prove the bus. Reset is done first so the ID read
 * happens from a known state. A wrong ID or scratchpad mismatch is FATAL: it
 * almost always means CS, SPI mode 0, 1 MHz, 3-/4-wire or a stuck hardware
 * reset is wrong.
 */
static int ad9144_reset_and_probe(const struct device *dev)
{
	...
	/*
	 * Release the AD9144 from hardware reset before any SPI access: the
	 * generic ZCU102 BOOT.BIN never drives this EMIO pin, so without this the
	 * chip stays in reset and every register reads back as bus float. Optional:
	 * boards that strap RESETB high omit the DT property.
	 */
 ...
 * SERDES-optimisation tables. pd_dac/pd_clk are derived from M: for M=2 they
 * resolve to PWRCNTRL0=0x18, CLKCFG0=0x40.
 */
 ...
 * 2's complement. NCO mixing is off (DAQ2 frequency-center-shift is 0), so
 * mod_type = NONE.
 */
...
static int ad9144_setup_link(const struct device *dev)
{
	...
	/* NO_SUBCLASS bit set only for subclass 0; BIT2 set = capture on rising edge. */
	...
	/* Geometry-derived ILAS bytes (scrambling on). */
...
static int ad9144_setup_datalink_sync(const struct device *dev)
{
	...
	/* Also program the link-1 LMFC registers. */
 ...
 * enable reads 0x00 and never locks. The DAQ2 FSM calls this from the AD9144
 * CLOCKS_ENABLE callback, which is visited after the transceiver's GT release.
 */
 ...
 * reported and setup continues (best-effort, log only).
 */
...
/* Full AD9144 setup (DAQ2 path), grouped into blocks. */
...
const struct jesd204_dev_data ad9144_jesd204_data = {
	.max_num_links = 1,
	.state_ops = {
