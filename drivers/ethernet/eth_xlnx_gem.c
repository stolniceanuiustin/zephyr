...
static int eth_xlnx_gem_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	reg_ctrl = (mem_addr_t)(&dev_data->tx_bd_ring.first_bd[curr_bd_idx].ctrl);
...
static void eth_xlnx_gem_configure_buffers(const struct device *dev)
{
	...
	for (buf_iter = 0; buf_iter < (dev_conf->rx_bd_count - 1); buf_iter++) {
		uint32_t addr = (uint32_t)(uintptr_t)dev_data->first_rx_buffer +
	...
	uint32_t last_rx_addr = (uint32_t)(uintptr_t)dev_data->first_rx_buffer +
				(buf_iter * dev_conf->rx_buffer_size);
	...
	for (buf_iter = 0; buf_iter < (dev_conf->tx_bd_count - 1); buf_iter++) {
		bdptr->addr = (uint32_t)(uintptr_t)dev_data->first_tx_buffer +
	...
	bdptr->addr = (uint32_t)(uintptr_t)dev_data->rx_tie_off_buffer |
		      ETH_XLNX_GEM_RX_BD_USED_BIT | ETH_XLNX_GEM_RX_BD_WRAP_BIT;
	bdptr->ctrl = 0x00000000;

	bdptr = dev_data->tx_bd_ring.tie_off_bd;
	bdptr->addr = (uint32_t)(uintptr_t)dev_data->tx_tie_off_buffer;
	bdptr->ctrl = ETH_XLNX_GEM_TX_BD_WRAP_BIT | ETH_XLNX_GEM_TX_BD_USED_BIT;
	 ...
	 */
	sys_write32((uint32_t)(uintptr_t)dev_data->rx_bd_ring.first_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_RXQBASE_OFFSET);
	sys_write32((uint32_t)(uintptr_t)dev_data->tx_bd_ring.first_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_TXQBASE_OFFSET);
#ifdef CONFIG_SOC_XILINX_ZYNQMP
	sys_write32(0x00000000, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_RX1QBASEH_OFFSET);
	sys_write32((uint32_t)(uintptr_t)dev_data->rx_bd_ring.tie_off_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_RX1QBASEL_OFFSET);
	sys_write32(0x00000000, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_TX1QBASEH_OFFSET);
	sys_write32((uint32_t)(uintptr_t)dev_data->tx_bd_ring.tie_off_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_XLNX_GEM_TX1QBASEL_OFFSET);
#endif /* CONFIG_SOC_XILINX_ZYNQMP */
}
...
static void eth_xlnx_gem_handle_rx_pending(const struct device *dev)
{
	...
	while (1) {
		...
		reg_addr = (mem_addr_t)(&dev_data->rx_bd_ring.first_bd[first_bd_idx].addr);
		reg_ctrl = (mem_addr_t)(&dev_data->rx_bd_ring.first_bd[first_bd_idx].ctrl);
		 ...
		 */
		do {
			reg_ctrl = (mem_addr_t)(&dev_data->rx_bd_ring.first_bd[last_bd_idx].ctrl);
			reg_val  = sys_read32(reg_ctrl);
		...
		do {
			if (pkt != NULL) {
				...
				sys_cache_data_invd_range(
					...
					dev_conf->rx_buffer_size);
#endif
				net_pkt_write(pkt, (const void *)(uintptr_t)
					      (dev_data->rx_bd_ring.first_bd[curr_bd_idx].addr &
					      ETH_XLNX_GEM_RX_BD_BUFFER_ADDR_MASK),
			 ...
			 */
			reg_addr = (mem_addr_t)(&dev_data->rx_bd_ring.first_bd[curr_bd_idx].addr);
			reg_val	 = sys_read32(reg_addr);
